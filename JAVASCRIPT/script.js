...
async function getMovieBySearch (search_item) {
  const response = await fetch(`https://api.themoviedb.org/3/search/movie?api_key=${API_KEY}&query=${search_item}`)
...
async function show_popup(card) {
  popup_container.classList.add('show-popup')
    ...
    popup_container.style.background = `linear-gradient(rgba(0, 0, 0, .9), rgba(0, 0, 0, 0.9)), url(${image_path + movie.poster_path})`
...
async function get_trending_movies () {
  const resp = await fetch(`https://api.themoviedb.org/3/trending/all/week?api_key=${API_KEY}`)
  const respData = await resp.json()
...
async function add_to_dom_trending () {
  ...
  trending_el.innerHTML = data.slice(0,70).map(e =>{
    // console.log(e)
...
async function get_series_movies () {
  const response = await fetch(`https://api.themoviedb.org/3/tv/?api_key=${API_KEY}`)
  const responseData = await response.json()
  return responseData.results
}
...
async function add_to_dom_series (){
  ...
  series.innerHTML = data.slice(0,10).map(e =>{
     console.log(e)
